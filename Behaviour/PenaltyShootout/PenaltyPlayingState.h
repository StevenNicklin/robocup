...
#ifndef PENALTY_PLAYING_FSM_STATE_H
#define PENALTY_PLAYING_FSM_STATE_H

class PenaltyShootoutProvider;
#include "PenaltyShootoutFSMState.h"

class PenaltyChaseState;
class PenaltyPositioningState;
class PenaltyBallIsLostState;
class PenaltyImLostState;

#include <vector>
using namespace std;

class PenaltyPlayingState : public PenaltyShootoutFSMState
{
public:
    PenaltyPlayingState(PenaltyShootoutProvider* provider);
    virtual ~PenaltyPlayingState();
protected:
    BehaviourFSMState* nextState();
    virtual void doStateCommons();
    virtual BehaviourFSMState* nextStateCommons();
    PenaltyChaseState* m_chase_state;
    PenaltyPositioningState* m_positioning_state;
    PenaltyBallIsLostState* m_ball_is_lost_state;
    PenaltyImLostState* m_im_lost_state;
private:
